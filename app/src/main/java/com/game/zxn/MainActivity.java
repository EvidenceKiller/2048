...
import com.game.zxn.settings.SettingsActivity;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (size == view.game.numSquaresX) {
            Tile[][] field = view.game.mGrid.field;
            ...
            for (int xx = 0; xx < saveState.length; xx++) {
                ...
                for (int yy = 0; yy < array.length; yy++) {
                    if (!array[yy].startsWith("0")) {
                        view.game.mGrid.field[xx][yy] = new Tile(xx, yy, Integer.valueOf(array[yy]));
                    } else {
                        view.game.mGrid.field[xx][yy] = null;
                    }
                }
            }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (view.inverseMode) {
            ...
            menu.findItem(R.id.menu_undo).setEnabled(view.game.mGrid.canRevert);
