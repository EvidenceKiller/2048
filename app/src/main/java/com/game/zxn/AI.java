...
/**
 * Created by ZXN on 2017/3/14 0014.
 */
public class AI {
    enum Player {
        DOCTOR,
    ...
    public int getBestMove() {
        ...
        do {
            int move = (int) search(mGame.clone(), depth, -10000, 10000, Player.DOCTOR)[0];
            if (move == -1) {
                break;
            } else {
                bestMove = move;
        ...
        if (player == Player.DOCTOR) {
            ...
            for (int i = 0; i <= 3; i++) {
                ...
                if (depth == 0) {
                    ...
                    score = (int) search(g, depth - 1, bestScore, beta, Player.DALEKS)[1];
            ...
            for (Object[] obj : conditions) {
                ...
                g.mGrid.insertTile(t);

                // Pass the game to human
                int score = (int) search(g, depth, alpha, bestScore, Player.DOCTOR)[1];
    ...
    private int evaluate(MainGame game) {
        ...
        int empty = game.mGrid.getAvailableCells().size();
            ...
            for (int y = 0; y < game.numSquaresY; y++) {
                ...
                if (t != null) {
                    ...
                    for (int direction = 1; direction <= 2; direction++) {
                        ...
                        if (game.mGrid.isCellOccupied(targetCell)) {
                            Tile target = game.mGrid.getCellContent(targetCell);
    ...
    private int getMonotonticity(MainGame game) {
        ...
        for (int x = 0; x < game.numSquaresX; x++) {
            ...
            while (next < game.numSquaresY) {
                while (next < game.numSquaresY && game.mGrid.isCellAvailable(new Cell(x, next))) {
                    next++;
                }
                if (next >= game.numSquaresY) {
                    next--;
                }
                int currentValue = game.mGrid.isCellOccupied(new Cell(x, current)) ?
                        (int) (Math.log(game.mGrid.getCellContent(x, current).getValue()) / Math.log(2)) :
                        0;
                int nextValue = game.mGrid.isCellOccupied(new Cell(x, next)) ?
                        (int) (Math.log(game.mGrid.getCellContent(x, next).getValue()) / Math.log(2)) :
                        0;
        ...
        for (int y = 0; y < game.numSquaresY; y++) {
            ...
            while (next < game.numSquaresX) {
                while (next < game.numSquaresX && game.mGrid.isCellAvailable(new Cell(next, y))) {
                    next++;
                }
                if (next >= game.numSquaresX) {
                    next--;
                }
                int currentValue = game.mGrid.isCellOccupied(new Cell(current, y)) ?
                        (int) (Math.log(game.mGrid.getCellContent(current, y).getValue()) / Math.log(2)) :
                        0;
                int nextValue = game.mGrid.isCellOccupied(new Cell(next, y)) ?
                        (int) (Math.log(game.mGrid.getCellContent(next, y).getValue()) / Math.log(2)) :
                        0;
                if (currentValue > nextValue) {
    ...
    private int getMaxValue(MainGame game) {
        ...
        for (int x = 0; x < game.numSquaresX; x++) {
            for (int y = 0; y < game.numSquaresY; y++) {
                ...
                if (game.mGrid.isCellOccupied(cell)) {
                    Tile t = game.mGrid.getCellContent(cell);
    ...
    private int countIslands(MainGame game) {
        ...
        for (int x = 0; x < game.numSquaresX; x++) {
            for (int y = 0; y < game.numSquaresY; y++) {
                if (game.mGrid.isCellOccupied(new Cell(x, y))) {
                    game.mGrid.getCellContent(x, y).marked = false;
                }
            }
        }
        ...
        for (int x = 0; x < game.numSquaresX; x++) {
            for (int y = 0; y < game.numSquaresY; y++) {
                if (game.mGrid.isCellOccupied(new Cell(x, y))) {
                    Tile t = game.mGrid.getCellContent(x, y);
                    if (!t.marked) {
                        islands++;
    ...
    private void mark(MainGame game, int x, int y, int value) {
        if (game.mGrid.isCellWithinBounds(x, y) && game.mGrid.isCellOccupied(new Cell(x, y))) {
            Tile t = game.mGrid.getCellContent(x, y);
